            ...
            if response.status_code == 200:
                ...
                for market in response.json():
                    ...
                    if market['symbol'] == normalized_symbol:
                        ...
                        # 處理科學計數法
                        min_qty = lot_size.get('minQty', '0.01')
                        min_order_size = float(min_qty) if 'e' not in min_qty else float(f"{float(min_qty):.8f}")
                    
                        return {
                            'base_precision': len(min_qty.split('.')[-1].rstrip('0')),
